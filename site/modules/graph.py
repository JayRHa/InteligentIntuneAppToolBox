...
def load_apps(access_token):
    ...
    response = requests.get(
        "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps?$filter=(microsoft.graph.managedApp/appAvailability%20eq%20null%20or%20microsoft.graph.managedApp/appAvailability%20eq%20%27lineOfBusiness%27%20or%20isAssigned%20eq%20true)&$orderby=displayName&",
        headers=headers,
    )
...
def get_group_name(group_id, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    response = requests.get(
        f"https://graph.microsoft.com/v1.0/groups/{group_id}", headers=headers
    )
    response.raise_for_status()
...
def get_app_assignments(app_id, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    response = requests.get(
        f"https://graph.microsoft.com/v1.0/deviceAppManagement/mobileApps/{app_id}/assignments",
        headers=headers,
    )
    response.raise_for_status()
    ...
    for assignment in assignments:
        ...
        if group_id != "N/A":
            try:
                assignment["groupName"] = get_group_name(
...
def get_installation_status(app_id, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    url = "https://graph.microsoft.com/beta/deviceManagement/reports/getAppStatusOverviewReport"
...
def search_groups(query, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    url = f"https://graph.microsoft.com/v1.0/groups?$filter=startswith(displayName,'{query}')"
...
def add_assignment(app_id, group_id, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    url = f"https://graph.microsoft.com/v1.0/deviceAppManagement/mobileApps/{app_id}/assignments"
    body = {
        "target": {
...
def remove_assignment(app_id, assignment_id, access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
    url = f"https://graph.microsoft.com/v1.0/deviceAppManagement/mobileApps/{app_id}/assignments/{assignment_id}"
...
def set_description(selected_app, description, access_token):
    ...
    url = f"https://graph.microsoft.com/beta/deviceAppManagement/mobileApps/{id_nr}"
